...
class DiagnosticRedactionTests(unittest.TestCase):
    ...
    def test_windows_absolute_logd_reference_fails_clearly(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            diagnostic_dir = root / "diagnostic"
            diagnostic_dir.mkdir()
            metadata_path = diagnostic_dir / "build-deadbeef.json"
            metadata_path.write_text(
                json.dumps({"diagnostic_logd": r"C:\Users\builder\build-deadbeef.logd"}),
                encoding="utf-8",
            )

            with patched_diagnostic_root(root, diagnostic_dir):
                with self.assertRaisesRegex(build.DiagnosticValidationError, "repo-relative"):
                    build.validate_diagnostic_pair(metadata_path)

    def test_posix_absolute_logd_reference_fails_clearly_on_every_platform(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            diagnostic_dir = root / "diagnostic"
            diagnostic_dir.mkdir()
            metadata_path = diagnostic_dir / "build-deadbeef.json"
            metadata_path.write_text(
                json.dumps({"diagnostic_logd": "/tmp/build-deadbeef.logd"}),
                encoding="utf-8",
            )
    ...
    def test_empty_logd_list_entry_fails_clearly(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            diagnostic_dir = root / "diagnostic"
            diagnostic_dir.mkdir()
            metadata_path = diagnostic_dir / "build-deadbeef.json"
            (diagnostic_dir / "build-deadbeef-part001.logd").write_bytes(b"logd")
            metadata_path.write_text(
                json.dumps({"diagnostic_logd": ["diagnostic/build-deadbeef-part001.logd", ""]}),
                encoding="utf-8",
            )

            with patched_diagnostic_root(root, diagnostic_dir):
                with self.assertRaisesRegex(build.DiagnosticValidationError, "non-empty"):
                    build.validate_diagnostic_pair(metadata_path)

    def test_chunked_logd_reference_requires_every_part(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            diagnostic_dir = root / "diagnostic"
            diagnostic_dir.mkdir()
            ...
            with patched_diagnostic_root(root, diagnostic_dir):
                with self.assertRaisesRegex(build.DiagnosticValidationError, "part002"):
                    build.validate_diagnostic_pair(metadata_path)

    def test_external_artifact_path_is_reduced_to_filename(self):
        outside_artifact = Path(tempfile.gettempdir()) / "outside" / "service.bin"
        report = build.build_diagnostic_report(
            [("backend", True, 1.25, "", str(outside_artifact))],
            "deadbeef",
            logd_relpaths=["diagnostic/build-deadbeef.logd"],
            password="test-password",
        )

        self.assertEqual(report["modules"][0]["artifact"], "external/service.bin")
        self.assertNotIn(tempfile.gettempdir(), json.dumps(report, sort_keys=True))
